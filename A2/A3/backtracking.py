...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    if unAssignedVars.empty():
        ...
        for var in csp.variables():
          temp = (var, var.getValue())
          answer.append(temp)
        return [answer]
    bt_search.nodesExplored += 1
    answers = []
    nxt_var = unAssignedVars.extract()
    if trace:
      print ("Trying {}".format(nxt_var.name()))
    for val in nxt_var.curDomain():
        if trace:
          print ("{} = {}".format(nxt_var.name(), val))
        nxt_var.setValue(val)
        flag = True
        for cons in csp.constraintsOf(nxt_var):
            if cons.numUnassigned() == 1:
                if FCCheck(cons, nxt_var, val) == "DWO":
                    flag = False
                    if trace: print "DWO occured\n"
                    break
        if flag:
            new_answers = FC(unAssignedVars, csp, allSolutions, trace)
            if new_answers:
                answers.extend(new_answers)
            if len(answers) > 0 and not allSolutions:
                nxt_var.restoreValues(nxt_var, val)
                break
        nxt_var.restoreValues(nxt_var, val)
    nxt_var.unAssign()
    unAssignedVars.insert(nxt_var)
    return answers
        ...
        for var in cons.scope():
            for val in var.curDomain():
                if not cons.hasSupport(var, val):
                    ...
                    if var.curDomainSize() == 0:
                        return "DWO"
                    for check in csp.constraintsOf(var):
                        if check not in constraints and check is not cons:
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    bt_search.nodesExplored += 1
    answers = []
    new_var = unAssignedVars.extract()
    if trace:
      print "Trying {}".format(new_var.name())
    for val in new_var.curDomain():
        if trace:
          print "{} = {}".format(new_var.name(), val)
        new_var.setValue(val)
        flag = True
        if GacEnforce(csp.constraintsOf(new_var), csp, new_var, val) == "DWO":
            flag = False
        if flag:
            new_answers = GAC(unAssignedVars, csp, allSolutions, trace)
            if new_answers:
                answers.extend(new_answers)
            if not allSolutions and len(answers) > 0:
                new_var.restoreValues(new_var, val)
                break  
                
        new_var.restoreValues(new_var, val)
    new_var.unAssign()
    unAssignedVars.insert(new_var)
    return answers
