...
def solve_planes(planes_problem, algo, allsolns,
    ...
    for plane in planes_problem.planes:
        start_var = Variable("Plane:{}_1".format(plane), planes_problem.can_start(plane) + ['Empty']) # initialize the start variable with its location
        ...
        for i in range(2, len(fly_plane)):
            new = Variable("Plane:{}_{}".format(plane, i), fly_plane) # initialize other variables that they can fly
            dic[plane].extend([new])
        block.extend(dic[plane])

        follow_plane = []
        for (f1, f2) in planes_problem.can_follow: # store all the flights that cna be followed
        ...
        for i in range(len(cols) - 1):
          new_con = TableConstraint("Plane_{}_({},{})".format(plane, i + 1, i + 2), [cols[i], cols[i + 1]], follow_plane)
        ...
        for i in range(len(cols) - maintain_freq + 1):
            name = "Plane_{}_Range({},{})".format(plane, i + 1, i + maintain_freq)
            scope = []
            ...
            for x in range(i, i + maintain_freq):
                scope.append(cols[x])
            lb = 1
            maintenance_constraint = NValuesConstraint(name, scope, required_values, lb, maintain_freq)
            cons.append(maintenance_constraint) # add the constraints for maintain flights
    ...
    for flight in planes_problem.flights:
        ...
        lb = 1
        ub = 1
        flight_constrain = NValuesConstraint(name, block, required_values, lb, ub)
    ...
    for item in solutions:
        ...
        for (var, val) in item:
            ...
            final_sol[var_name.split(":")[1].split("_")[0]].append((row_num, val))
        ordered_sol = {}
