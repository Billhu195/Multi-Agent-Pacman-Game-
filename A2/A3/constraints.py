...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for x in qi._dom:
            for y in qj._dom:
                if abs(x - y) != abs(i - j) and x != y:
                    tuples.append([x, y])
                    tuples.append([y, x])

        TableConstraint.__init__(self, name, lst, tuples)
...
class NValuesConstraint(Constraint):
    '''NValues constraint over a set of variables.
       ...
       are assigned the value 3, and at most 3 of them have been assigned the cd Desktop/CSC384 3
       '''
    ...
    def hasSupport(self, var, val):
        ...
        remain_vars = self.scope()
        remain_vars.remove(var)
        x = findvals(remain_vars, [(var, val)], satisfiesBounds, satisfiesUpperbound)
        return x
