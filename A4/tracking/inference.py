...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for prev_pos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, prev_pos))
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += self.beliefs[prev_pos] * prob
        self.beliefs = allPossible
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i % total_len])
        "*** END YOUR CODE HERE ***"
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particle in self.particles:
                true_dis = util.manhattanDistance(particle, pacmanPosition)
                weight[particle] += emissionModel[true_dis]
            weight.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        result = []
        for particle in self.particles:
            pos_dist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            result.append(util.sample(pos_dist))
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        beliefs_dis = util.Counter()
        for particle in self.particles:
            beliefs_dis[particle] += 1.0
        beliefs_dis.normalize()
        return beliefs_dis
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for index in range(self.numGhosts):
            if noisyDistances[index] is None:
                new_particles = []
                for particle in self.particles:
                    new_particles.append(self.getParticleWithGhostInJail(particle, index))
        ...
        for particle in self.particles:
            ...
            for index in range(self.numGhosts):
                if noisyDistances[index] is not None:
                    dis = util.manhattanDistance(pacmanPosition, particle[index])
                    weights[particle] = emissionModels[index][dis] * weights[particle]

        if weights.totalCount() == 0.0:
            self.initializeParticles()
            for g_index in range(self.numGhosts):
                if noisyDistances[g_index] is None:
                    for p_index in range(self.numParticles):
                        self.particles[p_index] = self.getParticleWithGhostInJail(self.particles[p_index], g_index)
        else:
            belief_dist = self.getBeliefDistribution()
            for particle in self.particles:
                belief_dist[particle] = weights[particle] * belief_dist[particle]
                weights[particle] = 1.0
            belief_dist.normalize()
            result = []
            for p_index in range(self.numParticles):
                result.append(util.sample(belief_dist))
            self.particles = result
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for index in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), index,
