...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint("all_diff_of_row", row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint("all_diff_of_column", scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint("all_diff_of_square", scope))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    # a list to keep all variables in csp
    all_var_array = []
    # a list to keep all constraints
    constraints = []
    # a list to keep all possible flights for every plane (a list of list)
    planes = []
    maintenance_freq = planes_problem.min_maintenance_frequency

    # create variables
    plane_index = 0
    for plane in planes_problem.planes:
        # for every plane, there are a list of flights, initial an empty list
        planes.append([])
        for index in range(len(planes_problem._can_fly[plane])):
            # first time using can_start as domain
            if index == 0:
                can_start = planes_problem.can_start(plane)
                # adding a new flight "end"
                can_start.append("end")
                name = plane + "@" + str(index)
                plane_flight = Variable(name ,can_start)
                planes[plane_index].append(plane_flight)
                all_var_array.append(plane_flight)
            else: # using can_fly as domain
                can_fly = []
                for item in planes_problem._can_fly[plane]:
                    can_fly.append(item)
                # adding a new flight "end"
                can_fly.append("end")
                name = plane + "@" + str(index)
                plane_flight = Variable(name, can_fly)
                planes[plane_index].append(plane_flight)
                all_var_array.append(plane_flight)
        plane_index += 1

    # create constrains
    # constraints for every flight only be scheduled once
    for flight in planes_problem.flights:
        # the lower bound and upper bound for NValuesConstraint should be 1
        constraint = NValuesConstraint("exactly once for " + str(flight), all_var_array, [flight], 1, 1)
        constraints.append(constraint)

    # constraints for checking that each flight which taken by a plane is a volid flight for that plane
    valid_flights = []
    for flight1, flight2 in planes_problem.can_follow:
        valid_flights.append((flight1, flight2))
    for flight in planes_problem.flights:
        valid_flights.append((flight, "end"))
    valid_flights.append(("end", "end"))

    for plane_i in range(len(planes)):
        for i in range(len(planes[plane_i]) - 1):
            constraint = TableConstraint("flight"+ str(plane_i), [planes[plane_i][i], planes[plane_i][i+1]], valid_flights)
            constraints.append(constraint)

    # constraints for checking maintenance
    for plane_i in range(len(planes)):
        if len(planes[plane_i]) >= maintenance_freq:
            for i in range(len(planes[plane_i]) - maintenance_freq + 1):
                scope = planes[plane_i][i: i + maintenance_freq]
                except_value = [flight for flight in planes_problem.maintenance_flights]
                except_value += ["end"]
                constraint = NValuesConstraint("maintenance" + str(plane_i), scope, except_value, 1, maintenance_freq)
                constraints.append(constraint)

    csp = CSP("Plane Problem", all_var_array, constraints)
    ...
    for solution in solutions:
        ...
        temp = []
        for plane_i in range(len(planes)):
            result = [planes_problem.planes[plane_i]]
            m = 0
            while m < len(planes[plane_i]):
                element = solution.pop(0)[1]
                if element != "end":
