...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    if unAssignedVars.empty():
        ...
        for vari in csp.variables():
            result.append((vari, vari.getValue()))
        return [result]
        ...
        for constraint in csp.constraintsOf(var):
            if constraint.numUnassigned() == 1:
                if FCCheck(constraint, var, val) == "DWO":
                    ...
                    break
        if noDWO:
            result2 = FC(unAssignedVars, csp, allSolutions, trace)
            if result2:
                results.extend(result2)
            if len(results) >= 1 and not allSolutions:
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in var.curDomain():
        ...
        if noDWO:
            result2 = GAC(unAssignedVars, csp, allSolutions, trace)
            results.extend(result2)
            if len(results) >= 1 and not allSolutions:
                var.restoreValues(var, val)
                break
