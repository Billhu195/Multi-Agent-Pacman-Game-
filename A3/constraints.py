...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        valid = []
        for i_index in qi.domain():
            for j_index in qj.domain():
                if i_index != j_index and abs(i_index - j_index) != abs(i - j):
                    valid.append([i_index, j_index])
        TableConstraint.__init__(self, name, [qi,qj], valid)
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for variable in self.scope():
            if variable.getValue() == self._required:
                count += 1
    ...
    def hasSupport(self, var, val):
        ...
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], full_test, partial_test)
        return x
